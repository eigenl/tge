...
#define LunaMethod(name) {#name, &__LUNA_CLASS::name}

#define LUNA_CLASS_HEADER static const char className[]; \
                          static const Luna<LUA_OBJ_CLASS>::FunctionType methods[]; \
                          static const Luna<LUA_OBJ_CLASS>::PropertyType properties[]; \
                          bool isExisting; \
                          bool isPrecious; \
                          LUA_OBJ_CLASS(lua_State *L) { } \
                          ~LUA_OBJ_CLASS() { }

#define LUNA_CLASS_HEADER_INSTANCE  OBJ_CLASS * obj; \
                                    static const char className[]; \
                                    static const Luna<LUA_OBJ_CLASS>::FunctionType methods[]; \
                                    static const Luna<LUA_OBJ_CLASS>::PropertyType properties[]; \
                                    bool isExisting; \
                                    bool isPrecious; \
                                    LUA_OBJ_CLASS(lua_State *L) { obj = (OBJ_CLASS *)lua_touserdata(L, 1); } \
                                    ~LUA_OBJ_CLASS() { } \
                                    int instance(lua_State * L) { if (!obj) lua_pushnil(L); else lua_pushlightuserdata(L, obj); return 1; }
